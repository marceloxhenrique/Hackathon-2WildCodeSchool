...
class PhoneManager extends AbstractManager {
  ...
  find(phoneId) {
    return this.database.query(
      `
      SELECT phone.id, phone.charger, os.id, os.os, os.version, brand.id, brand.brand, model.id, model.model, model.indice, color.id, color.color, memory.id, memory.memory, memory.val_m, storage.id, storage.storage, storage.val_s, state.id, state.state, state.ponderation, category.id, category.category, category.val_min, category.val_max, network.id, network.network, screen_size.id, screen_size.size
      FROM phone
